...
echo -n > /etc/akubudakgerik/cache/trojan_temp.txt;
echo -n > /etc/akubudakgerik/cache/trojan_temp2.txt;
...
if [[ "$jum" = "$ip" ]]; then
echo "$jum" >> /etc/akubudakgerik/cache/trojan_temp.txt;
else
echo "$ip" >> /etc/akubudakgerik/cache/trojan_temp2.txt;
fi
jum2=$(cat /etc/akubudakgerik/cache/trojan_temp.txt);
sed -i "/$jum2/d" /etc/akubudakgerik/cache/trojan_temp2.txt > /dev/null 2>&1;
done
jum=$(cat /etc/akubudakgerik/cache/trojan_temp.txt);
if [[ -z "$jum" ]]; then
echo > /dev/null;
else
jum2=$(cat /etc/akubudakgerik/cache/trojan_temp.txt | nl);
hariini=`date -d "0 days" +"%Y-%m-%d"`;
echo "Username : $akun";
echo "$jum2";
echo ""
fi
rm -rf /etc/akubudakgerik/cache/trojan_temp.txt;
done
echo "=============================================";
echo "            GRPC IP Login List";
echo "=============================================";
oth=$(cat /etc/akubudakgerik/cache/trojan_temp2.txt | sort | uniq | nl );
echo "GRPC IP : All User IP";
echo "$oth";
echo "=============================================";
echo ""
rm -rf /etc/akubudakgerik/cache/trojan_temp2.txt;