...
echo -n > /etc/akubudakgerik/cache/vless_temp.txt;
echo -n > /etc/akubudakgerik/cache/vless_temp2.txt;
...
for ip in "${data2[@]}"
do
jum=$(cat /etc/akubudakgerik/xray-mini-tls/access.log | grep -w $akun | awk '{print $3}' | cut -d: -f1 | grep -w $ip | sort | uniq);
if [[ "$jum" = "$ip" ]]; then
echo "$jum" >> /etc/akubudakgerik/cache/vless_temp.txt;
else
echo "$ip" >> /etc/akubudakgerik/cache/vless_temp2.txt;
fi
jum2=$(cat /etc/akubudakgerik/cache/vless_temp.txt);
sed -i "/$jum2/d" /etc/akubudakgerik/cache/vless_temp2.txt > /dev/null 2>&1;
done
jum=$(cat /etc/akubudakgerik/cache/vless_temp.txt);
if [[ -z "$jum" ]]; then
echo > /dev/null;
else
jum2=$(cat /etc/akubudakgerik/cache/vless_temp.txt | nl);
...
echo "=============================================";
echo -n > /etc/akubudakgerik/cache/vless_temp3.txt;
data=(`cat /etc/xray-mini/client.conf | grep '^Vless' | cut -d " " -f 2`);
for akun in "${data[@]}"
do
if [[ -z "$akun" ]]; then
...
ip=$( cat /etc/akubudakgerik/xray-mini-nontls/access.log | grep "$(date -d "0 days" +"%H:%M" )" | grep -w $akun | tail -n100 | awk '{print $3}' | cut -d: -f1 | sort | uniq );
if [[ -z "$ip" ]]; then
echo > /dev/null;
else
jum5=$(echo $ip | nl);
...
echo "=============================================";
oth=$(cat /etc/akubudakgerik/cache/vless_temp2.txt | sort | uniq | nl );
echo "GRPC IP : All User IP";
echo "$oth";
echo "=============================================";
echo "";
rm -rf /etc/akubudakgerik/cache/vless_temp2.txt;
rm -rf /etc/akubudakgerik/cache/vless_temp.txt;
rm -rf /etc/akubudakgerik/cache/vless_temp3.txt;