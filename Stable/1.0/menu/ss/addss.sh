...
cp /etc/xray-mini/shadowsocks.json /etc/akubudakgerik/xray-mini-utils/shadowsocks-backup.json;
cat /etc/akubudakgerik/xray-mini-utils/shadowsocks-backup.json | jq '.inbounds[0].settings.clients += [{"method": "chacha20-ietf-poly1305","password": "'${Username}'","email": "'${Username}'" }]' > /etc/xray-mini/shadowsocks.json;
...
mkdir -p /etc/akubudakgerik/shadowsocks/;
mkdir -p /etc/akubudakgerik/shadowsocks/${Username};
rm -f /etc/akubudakgerik/shadowsocks/${Username}/config.log;

# // Success
sleep 1;
clear;
echo -e "Your Premium Shadowsocks Details" | tee -a /etc/akubudakgerik/shadowsocks/${Username}/config.log;
echo -e "===============================" | tee -a /etc/akubudakgerik/shadowsocks/${Username}/config.log;
echo -e " Remarks     = ${Username}" | tee -a /etc/akubudakgerik/shadowsocks/${Username}/config.log;
echo -e " IP          = ${IP_NYA}" | tee -a /etc/akubudakgerik/shadowsocks/${Username}/config.log;
echo -e " Address     = ${domain}" | tee -a /etc/akubudakgerik/shadowsocks/${Username}/config.log;
echo -e " Port        = ${port}" | tee -a /etc/akubudakgerik/shadowsocks/${Username}/config.log;
echo -e " Password    = ${Username}" | tee -a /etc/akubudakgerik/shadowsocks/${Username}/config.log;
echo -e "===============================" | tee -a /etc/akubudakgerik/shadowsocks/${Username}/config.log;
echo -e " SHADOWSOCKS CONFIG LINK" | tee -a /etc/akubudakgerik/shadowsocks/${Username}/config.log;
echo -e ' ```'${link_config}'```' | tee -a /etc/akubudakgerik/shadowsocks/${Username}/config.log;
echo -e "===============================" | tee -a /etc/akubudakgerik/shadowsocks/${Username}/config.log;
echo -e " Created     = ${hariini}" | tee -a /etc/akubudakgerik/shadowsocks/${Username}/config.log;
echo -e " Expired     = ${exp}";
echo -e "===============================";