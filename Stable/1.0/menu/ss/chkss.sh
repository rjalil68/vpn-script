...
echo -n > /etc/akubudakgerik/cache/shadowsocks_temp.txt;
echo -n > /etc/akubudakgerik/cache/shadowsocks_temp2.txt;
...
for ip in "${data2[@]}"
do
jum=$(cat /etc/akubudakgerik/xray-mini-shadowsocks/access.log | grep -w $akun | awk '{print $3}' | cut -d: -f1 | grep -w $ip | sort | uniq);
if [[ "$jum" = "$ip" ]]; then
echo "$jum" >> /etc/akubudakgerik/cache/shadowsocks_temp.txt;
else
echo "$ip" >> /etc/akubudakgerik/cache/shadowsocks_temp2.txt;
fi
jum2=$(cat /etc/akubudakgerik/cache/shadowsocks_temp.txt);
sed -i "/$jum2/d" /etc/akubudakgerik/cache/shadowsocks_temp2.txt > /dev/null 2>&1;
done
jum=$(cat /etc/akubudakgerik/cache/shadowsocks_temp.txt);
if [[ -z "$jum" ]]; then
echo > /dev/null;
else
jum2=$(cat /etc/akubudakgerik/cache/shadowsocks_temp.txt | nl);
hariini=`date -d "0 days" +"%Y-%m-%d"`;
echo "Username : $akun";
echo "$jum2";
echo "";
fi
rm -rf /etc/akubudakgerik/cache/shadowsocks_temp.txt;
done
echo "=============================================";
echo "";
rm -rf /etc/akubudakgerik/cache/shadowsocks_temp2.txt;