#!/bin/bash
# (C) Copyright 2922-2023 By akubudakgerik
# ==================================================================
...
# // Start
echo "$(date) Autokill Shadowsocks MultiLogin for akubudakgerik Script V1.0 Stable";
...
sleep 30
# // Start
grep -c -E "^Shadowsocks " "/etc/xray-mini/client.conf" > /etc/akubudakgerik/jumlah-akun-shadowsocks.db;
grep "^Shadowsocks " "/etc/xray-mini/client.conf" | awk '{print $2}'  > /etc/akubudakgerik/akun-shadowsocks.db;
totalaccounts=`cat /etc/akubudakgerik/akun-shadowsocks.db | wc -l`;
echo "Total Akun = $totalaccounts" > /etc/akubudakgerik/total-akun-trojan.db;
for((i=1; i<=$totalaccounts; i++ ));
do
    # // Username Interval Counting
    username=$( head -n $i /etc/akubudakgerik/akun-shadowsocks.db | tail -n 1 );
    ...
    jumlah_baris_log=$( cat /etc/akubudakgerik/xray-mini-shadowsocks/access.log | grep -w $hariini | grep -w $waktu | grep -w 'accepted' | grep -w $username | wc -l );
    ...
    if [[ $jumlah_baris_log -gt $SHADOWSOCKS ]]; then
        ...
        cat /etc/akubudakgerik/xray-mini-utils/shadowsocks-backup.json | jq 'del(.inbounds[0].settings.clients[] | select(.email == "'${username}'"))' > /etc/xray-mini/shadowsocks.json;
        rm -rf /etc/akubudakgerik/shadowsocks/${username};
        ...
        echo "SHADOWSOCKS - $(date) - ${username} - Multi Login Detected - Killed at ${date}" >> /etc/akubudakgerik/autokill.log;
    fi
# // End Function
done
done