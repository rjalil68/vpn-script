...
mkdir -p /etc/akubudakgerik/xray-mini-http/;
...
cat /etc/akubudakgerik/xray-mini-utils/http-backup.json | jq '.inbounds[0].settings.accounts += [{"user": "'${Username}'","pass": "'${Username}'" }]' > /etc/xray-mini/http.json;
...
mkdir -p /etc/akubudakgerik/http/;
mkdir -p /etc/akubudakgerik/http/${Username};
rm -f /etc/akubudakgerik/http/${Username}/config.log;

# // Success
sleep 1;
clear;
echo -e "Create From Panel | HTTP Proxy" | tee -a /etc/akubudakgerik/http/${Username}/config.log;
echo -e "===============================" | tee -a /etc/akubudakgerik/http/${Username}/config.log;
echo -e " IP          = ${IP_NYA}" | tee -a /etc/akubudakgerik/http/${Username}/config.log;
echo -e " Address     = ${domain}" | tee -a /etc/akubudakgerik/http/${Username}/config.log;
echo -e " Port        = ${port}" | tee -a /etc/akubudakgerik/http/${Username}/config.log;
echo -e " User        = ${Username}" | tee -a /etc/akubudakgerik/http/${Username}/config.log;
echo -e " Password    = ${Username}" | tee -a /etc/akubudakgerik/http/${Username}/config.log;
echo -e "===============================" | tee -a /etc/akubudakgerik/http/${Username}/config.log;
echo -e " Created     = ${hariini}" | tee -a /etc/akubudakgerik/http/${Username}/config.log;
echo -e " Expired     = ${exp}";
echo -e "===============================";