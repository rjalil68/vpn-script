...
# // Make Client Config
cat > /etc/akubudakgerik/wireguard-cache.tmp << END
[Interface]
...
mkdir -p /etc/akubudakgerik/wireguard/;
rm -rf /etc/akubudakgerik/wireguard/$Username;
mkdir -p /etc/akubudakgerik/wireguard/$Username;

# // Restarting Service & Copy Client data to webserver
systemctl restart "wg-quick@wg0";
sysctl -p
cp /etc/akubudakgerik/wireguard-cache.tmp /etc/akubudakgerik/webserver/wg-client/${Username}.conf;

# // Clear
clear;

# // Detail
echo -e "Your Premium Wireguard Details" | tee -a /etc/akubudakgerik/wireguard/${Username}/config.log;
echo -e "===============================" | tee -a /etc/akubudakgerik/wireguard/${Username}/config.log;
echo -e " Username    = ${Username}" | tee -a /etc/akubudakgerik/wireguard/${Username}/config.log;
echo -e " IP          = ${IP}" | tee -a /etc/akubudakgerik/wireguard/${Username}/config.log;
echo -e " Address     = ${Domain}" | tee -a /etc/akubudakgerik/wireguard/${Username}/config.log;
echo -e "===============================" | tee -a /etc/akubudakgerik/wireguard/${Username}/config.log;
echo -e " Config File = http://${IP}:85/wg-client/${Username}.conf" | tee -a /etc/akubudakgerik/wireguard/${Username}/config.log;
echo -e "===============================" | tee -a /etc/akubudakgerik/wireguard/${Username}/config.log;
echo -e " Created     = ${hariini}" | tee -a /etc/akubudakgerik/wireguard/${Username}/config.log;
echo -e " Expired     = ${exp}";
echo -e "===============================";