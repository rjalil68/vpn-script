...
rm -r -f /etc/akubudakgerik/ssr-server;
mkdir -p /etc/akubudakgerik/ssr-server;
...
wget -q -O /etc/akubudakgerik/ssr-server/SSR-Server.zip "https://rjalil68/vpn-script/Resource/Core/SSR-Server.zip";
unzip -o SSR-Server.zip > /dev/null 2>&1;
chmod +x jq;
rm -f SSR-Server.zip;
cp config.json /etc/akubudakgerik/ssr-server/user-config.json;
cp mysql.json /etc/akubudakgerik/ssr-server/usermysql.json;
cp apiconfig.py /etc/akubudakgerik/ssr-server/userapiconfig.py;
sed -i "s/API_INTERFACE = 'sspanelv2'/API_INTERFACE = 'mudbjson'/" /etc/akubudakgerik/ssr-server/userapiconfig.py;
sed -i "s/SERVER_PUB_ADDR = '127.0.0.1'/SERVER_PUB_ADDR = '${IP_NYA}'/" /etc/akubudakgerik/ssr-server/userapiconfig.py;
sed -i 's/ \/\/ only works under multi-user mode//g' /etc/akubudakgerik/ssr-server/user-config.json;
cd;
...
wget -q -O /etc/init.d/ssr-server "https://rjalil68/vpn-script/Resource/Service/ssr-server_init";
