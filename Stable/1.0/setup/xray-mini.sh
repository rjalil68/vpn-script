...
wget -q -O /usr/local/akubudakgerik/xray-mini "https://rjalil68/vpn-script/Resource/Core/xray-mini";
chmod +x /usr/local/akubudakgerik/xray-mini;

# // Downloading XRay Mini Service
wget -q -O /etc/systemd/system/xray-mini@.service "https://rjalil68/vpn-script/Resource/Service/xray-mini_service";
chmod +x /etc/systemd/system/xray-mini@.service;

# // Removing Old Folder
rm -rf /etc/xray-mini/;
rm -rf /etc/akubudakgerik/xray-mini-tls/;
rm -rf /etc/akubudakgerik/xray-mini-nontls/;
rm -rf /etc/akubudakgerik/xray-mini-shadowsocks/;
rm -rf /etc/akubudakgerik/xray-mini-socks/;
...
mkdir -p /etc/akubudakgerik/xray-mini-utils/;
mkdir -p /etc/akubudakgerik/xray-mini-shadowsocks/;
mkdir -p /etc/akubudakgerik/xray-mini-socks/;
touch /etc/xray-mini/client.conf;

# // Getting Domain Addres
export domain=$( cat /etc/akubudakgerik/domain.txt );

# // Downloading XRay TLS Config
wget -qO- "https://rjalil68/vpn-script/Resource/Xray-Mini/1.0.Stable/tls_json" | jq '.inbounds[0].streamSettings.xtlsSettings.certificates += [{"certificateFile": "'/root/.acme.sh/${domain}_ecc/fullchain.cer'","keyFile": "'/root/.acme.sh/${domain}_ecc/${domain}.key'"}]' > /etc/xray-mini/tls.json;
wget -q -O /etc/xray-mini/nontls.json "https://rjalil68/vpn-script/Resource/Xray-Mini/1.0.Stable/nontls_json";
wget -q -O /etc/xray-mini/shadowsocks.json "https://rjalil68/vpn-script/Resource/Xray-Mini/1.0.Stable/shadowsocks_json";
wget -q -O /etc/xray-mini/socks.json "https://rjalil68/vpn-script/Resource/Xray-Mini/1.0.Stable/socks_json";
wget -q -O /etc/xray-mini/http.json "https://rjalil68/vpn-script/Resource/Xray-Mini/1.0.Stable/http_json";
